...
'''
Component: Actuator 1. ID-311330500
...
def checkingTemp():
    ...
    if numOfSamples > 0:
        for j in range(0, numOfSamples):
            if input_temp.samples[j].get_number('sensorID') == 1:
                lastTemp = input_temp.samples.getNumber(j, "temp")
        if lastTemp is None:
            return None  # there is no temp received from sensor1
        elif lastTemp < 20 or lastTemp > 40:
